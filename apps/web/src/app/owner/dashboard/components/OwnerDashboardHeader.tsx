    ...
    <header className="sticky top-0 z-30 backdrop-blur bg-surface/80 border-b border-ink-100">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <Link
          ...
          aria-label="CarryMe — owner dashboard"
        ...
        <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-ink-500">
          <a className="text-brand-deep" href="/owner/dashboard">
            Dashboard
          </a>
          <a className="hover:text-brand-deep transition" href="#fleet">
            Fleet
          </a>
          <a className="hover:text-brand-deep transition" href="#withdraw">
            Withdraw
          </a>
        </nav>

        <HeaderActions user={user} initials={initials} />
      </div>
    </header>
  );
}
